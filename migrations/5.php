<?php
    ...
    if ($exists) {
        ...
        $GLOBALS['db']->execute("INSERT INTO `users` (`id`, `osu_id`, `username`, `password_md5`, `salt`, `email`, `register_datetime`, `rank`, `allowed`, `latest_activity`, `silence_end`, `silence_reason`) VALUES ('999', '999', 'Rohwabot', ?, ?, 'rohw@bo.t', '1452544880', '4', '1', '0', '0', '');", [$md5Password, base64_encode($options['salt'])]);
     ...
     $file = fopen('./botpasswd.txt', 'w');
     fwrite($file, $plainPassword);
     fclose($file);
    }
