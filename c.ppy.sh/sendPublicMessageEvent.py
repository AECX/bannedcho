...
def handle(userToken, packetData):
	...
	try:
		...
		fokaMessage = Rohwabot.RohwabotResponse(username, packetData["to"], packetData["message"])
		if fokaMessage != False:
			who.append(userID)
			glob.tokens.multipleEnqueue(serverPackets.sendMessage("Rohwabot", packetData["to"], fokaMessage), who, False)
			consoleHelper.printColored("> Rohwabot@{}: {}".format(packetData["to"], str(fokaMessage.encode("UTF-8"))), bcolors.PINK)
