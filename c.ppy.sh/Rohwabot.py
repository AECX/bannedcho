"""Rohwabot related functions"""
import userHelper
import glob
import actions
import serverPackets
import RohwabotCommands

def connect():
	"""Add Rohwabot to connected users and send userpanel/stats packet to everyone"""
...
def disconnect():
	"""Remove Rohwabot from connected users"""

	glob.tokens.deleteToken(glob.tokens.getTokenFromUserID(999))

def RohwabotResponse(fro, chan, message):
	"""
	Check if a message has triggered Rohwabot (and return its response)

	fro -- sender username (for permissions stuff with admin commands)
	chan -- channel name
	message -- message

	return -- Rohwabot's response string or False
	"""

	for i in RohwabotCommands.commands:
